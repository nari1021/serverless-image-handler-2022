...
class ImageRequest {
    ...
    setup(event) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                imageRequestInfo.bucket = this.parseImageBucket(event, imageRequestInfo.requestType, imageRequestInfo.key);
    ...
    parseImageBucket(event, requestType, image_key) {
        ...
        const target_bucket_name = image_key.split('/')[0];
        ...
        if (requestType === lib_1.RequestTypes.DEFAULT) {
            ...
            if (request.bucket !== undefined) {
                ...
                return this.checkImageBucket(sourceBuckets, target_bucket_name);
            }
        }
    ...
    parseImageKey(event, requestType) {
        ...
        if (requestType === lib_1.RequestTypes.THUMBOR || requestType === lib_1.RequestTypes.CUSTOM) {
            ...
            const result_path = decodeURIComponent(path.replace(/\/\d+x\d+:\d+x\d+\/|(?<=\/)\d+x\d+\/|filters:[^/]+|\/fit-in(?=\/)|^\/+/g, '').replace(/^\/+/, ''));
